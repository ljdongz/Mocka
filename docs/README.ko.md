...
- **Mock Endpoint 관리** — GET, POST, PUT, DELETE, PATCH 등 모든 HTTP 메서드로 endpoint 생성. 각 endpoint에 alias를 부여하여 쉽게 식별하고, `multipart/form-data` 등 다양한 Content-Type으로 요청 본문 정의 가능
...
- **조건부 매치 룰** — 수신 요청의 body, header, query param, path param을 기반으로 적절한 응답 변형을 자동 선택. equals, contains, startsWith, endsWith, regex 연산자와 AND/OR 조합 지원
...
- **커스텀 응답 헤더** — 응답 변형별로 커스텀 HTTP 헤더를 정의. 헤더 값에 환경 변수를 사용하면 요청 시점에 자동 치환
- **Import / Export** — 모든 endpoint, 응답 변형, collection을 JSON 파일로 내보내고 불러오기. 중복 처리 정책 지원 (건너뛰기, 덮어쓰기, 병합)
- **드래그 앤 드롭 정렬** — Collection과 endpoint를 드래그하여 원하는 순서로 정렬
- **실시간 요청 기록** — WebSocket을 통해 수신 요청을 실시간으로 모니터링
- **응답 지연 설정** — 초 단위로 지연 시간을 설정하여 네트워크 레이턴시 시뮬레이션. 우선순위: `x-mock-response-delay` 헤더 > 변형별 delay > 전역 기본값
- **다국어 UI** — 설정에서 English / 한국어 전환
- **라이트 / 다크 테마** — 밝은 테마와 어두운 테마 간 전환
